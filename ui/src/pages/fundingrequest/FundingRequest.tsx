...
import { FundingApproval, FundingContractRequest  } from "@daml.js/mortgage-app/lib/Mortgage";
...
export default function FundingRequestList() {
    const assets = useStreamQueries(Mortgage.FundingContractRequest).contracts;
    ...
    function showApproval(asset : Mortgage.FundingContractRequest.CreateEvent) {
      async function onClose(state : FundingApproval | null) {
        setApprovalProps({ ...defaultApprovalProps, open: false});
        if (!state) return;
        await ledger.exercise(Mortgage.FundingContractRequest.FundingApproval, asset.contractId, {...state, selectedGse: partyIdentifier(state.selectedGse)}); //BGY
        /* await ledger.exercise(Mortgage.FundingContractRequest.FundingApproval, asset.contractId, {...state, selectedgse: partyIdentifier(state.selectedgse)}); */
      };
