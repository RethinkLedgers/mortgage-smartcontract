...
import { HomeLoanContract } from "@daml.js/mortgage-app/lib/Mortgage";
import { FundingRequest } from "@daml.js/mortgage-app/lib/Mortgage";
...
export default function HomeLoanContractList() {
  ...
  const retailcontracts = useStreamQueries(Mortgage.HomeLoanContract).contracts;
  ...
  function showNewAsset() {
    async function onClose(state: InputFieldsForNewAsset | null) {
      ...
      await ledger.create(Mortgage.HomeLoanContract, efData);
    };
  ...
  function showRequest(asset: Mortgage.HomeLoanContract.CreateEvent) {
    async function onClose(state: FundingRequest | null) {
      setRequestProps({ ...defaultRequestProps, open: false });
      if (!state) return;
      await ledger.exercise(Mortgage.HomeLoanContract.FundingRequest, asset.contractId, {...state, newGse: partyIdentifier(state.newGse)}); //BGY
    };
